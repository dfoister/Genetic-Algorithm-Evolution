...
{
	if (type == Types::PLAYER) return "PLAYER";
	if (type == Types::MAP) return "MAP";
	if (type == Types::GUARD) return "GUARD";
	if (type == Types::PIXIE) return "PIXIE";
	if (type == Types::SPELL) return "SPELL";
	if (type == Types::FRUIT) return "FRUIT";
	if (type == Types::FROGGO) return "FROGGO";
	if (type == Types::LADDER) return "LADDER";
...
float CollisionBounds::vectorLength(Vector2f v)
{
	return sqrt((v.x() * v.x()) + (v.y() * v.y()));
}

Vector2f CollisionBounds::normalizedVec(Vector2f v)
{
	if (vectorLength(v) != 0) {

		float tempLength = 1.0f / vectorLength(v);
		float tempX;
		float tempY;
...
#
			...
			*/
			...
			float distance = vectorLength(localPoint);

			if (distance < col->getRadius()) {
				Vector2f normal = normalizedVec(localPoint);
		...
		if (col->getShape() == CollisionBounds::Shapes::CIRCLE) {
			...
			if (deltaLength < radii) {
				...
				Vector2f normal = normalizedVec(delta);
